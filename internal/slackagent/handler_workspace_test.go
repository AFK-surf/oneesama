...
func TestHandleWorkspaceBootstrapCreatesTemplates(t *testing.T) {
	...
	if !strings.Contains(response.Body.String(), `"template_count":23`) || !strings.Contains(response.Body.String(), `backfill_agent_read_prompt.en.tmpl`) || !strings.Contains(response.Body.String(), `related_memory_evidence.zh.tmpl`) {
