...
|---|---|---|
| `oneesama-live.sh` | Build the unified `oneesama` binary, start `slack-agent` + `meeting-agent` under tmux, run env preflight, and re-attach if the windows already exist. | The canonical deployment entry point. Reads `~/.oneesama-live.env` for live secrets. Idempotent: safe to re-run. |
...
| `oneesama-status-report.sh` | Run `oneesama-monitor.sh` + `oneesama-triage-quality-sweep.sh` into a shared output dir, then emit a merged `status-report.json` (schema `oneesama.status-report.v1`) and `status-report.md`. | Exit 0 only when both are ok. Output dir defaults to `mktemp -d`; override via `ONEESAMA_STATUS_REPORT_OUTPUT_DIR`. |
