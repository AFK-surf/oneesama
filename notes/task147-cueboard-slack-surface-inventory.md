...
| `image_generation_tool.go`, `audio_generation_tool.go` | Image/audio generation. | deliberately blocked/excluded | product-excluded | Peng explicitly said not to do `image_generation` / `audio_generation` for task #147. |
| `copilot_tools.go` | `send_meeting_chat`. | deliberately blocked/excluded | product-excluded | Peng explicitly said not to do `send_meeting_chat` for task #147. |
