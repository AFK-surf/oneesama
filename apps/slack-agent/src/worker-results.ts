...
export function createSlackWorkerResultHandlers(ctx: any) {
  const { config, finalizedWorkerJobReports, poster, postJson, finalizeSlackTriageJob, postSlackMessage, scheduleSlackAssistantThreadStatus, assistantStatusTextForJob } = ctx;
