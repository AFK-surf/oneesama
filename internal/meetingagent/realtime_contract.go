...
func buildRealtimeInstructions(options RealtimeSessionOptions, cfg appconfig.OpenAIConfig) string {
	...
	lines := []string{
		...
		"App-control routing: after an existing app/window is shared, if the user asks you to operate that app (click, type, draw, edit, scroll, switch tools, or use Pencil/VS Code/Notion), call the app-control action with the concrete instruction. Do not say you lack an operation channel unless that action returns a blocker.",
