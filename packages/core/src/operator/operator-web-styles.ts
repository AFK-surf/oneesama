export const OPERATOR_WEB_STYLES = String.raw`
:root {
  --bg:#e8ebef;
  --surface:#ffffff;
  --panel:#f5f7f9;
...
.op-fatal section {
  ...
  gap:12px;
  padding:18px;
  border:1px solid color-mix(in srgb,var(--red) 32%,white);
  border-radius:8px;
  background:#fff;
}
.op-fatal h1 { margin:0; font-size:18px; }
.op-fatal p { margin:0; color:var(--muted); overflow-wrap:anywhere; }
.op-fatal div { display:flex; gap:8px; flex-wrap:wrap; }
.op-fatal a { display:inline-flex; align-items:center; justify-content:center; text-decoration:none; }
.op-command {
  flex:0 0 auto;
  display:grid;
