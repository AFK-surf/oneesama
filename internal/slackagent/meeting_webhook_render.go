...
func buildMeetingResultPost(payload NormalizedMeetingWebhookPayload) string {
	...
	lines = appendMeetingList(lines, "Artifacts", meetingArtifactLines(payload.Artifacts))
	if len(lines) == 1 {
...
func buildMeetingCanvasMarkdown(payload NormalizedMeetingWebhookPayload) string {
	...
	appendCanvasAttachmentSection(&sb, meetingCanvasAttachmentLinks(payload.Artifacts))
