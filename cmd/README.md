...
| `oneesama-triage-benchmark` | Replay live Slack threads or labeled fixtures through the triage dry-run path and summarize expected-vs-actual quality signals; supports `--config-set` variant metadata for A/B reports. | read-only; dry-run endpoint only |
