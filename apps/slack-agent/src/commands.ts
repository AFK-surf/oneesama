...
export function createSlackCommandHandlers(ctx: any) {
  ...
  function buildAutoRollbackDecision({
    ...
    meetingAgent: {
      status?: number;
      body?: { error?: string; detail?: string; [key: string]: unknown };
    };
  }) {
    return {
      mode: "rollback",
    ...
    if (parsed.action === "join") {
      ...
      checks.push(
        { name: "delegate_task_present", pass: Boolean(parsed.task), task: parsed.task },
        {
          ...
          pass:
            stack.wouldStartWorker === Boolean(parsed.task) && stack.sideEffects === "suppressed",
        },
      );
    } else {
      checks.push({
  ...
  async function handleShadowSlackCommand({
    ...
    const event = await appendShadowTapEvent(
      config.shadowTapReportPath || config.cutoverReportPath,
      {
      ...
      const job = await runner.startTask({
        ...
        context: {
          ...
          localSlackMemory: localSlackMemory.buildAgentContext({
            query: [memoryQuery, slackContext?.channelName, body.user_name]
              .filter(Boolean)
              .join(" "),
            limit: 5,
          }),
