 ...
 * Live screen-capture frames need Screen Recording TCC; for now frames are not
 * streamed.
 */
export function createLanOperatorWorkRuntime(
  ...
  const createPlanner =
    options.createPlanner ??
    ((hint: string) =>
      createOpenAIWorkPlanner({
        startHint: `You are operating the macOS app "${hint}" via Accessibility. "navigate" is unavailable; work with the current window and refs.`,
      }));
  let running = false;
