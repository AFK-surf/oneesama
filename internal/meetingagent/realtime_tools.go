...
func defaultRealtimeToolDefinitions() []realtimeToolSchema {
	return []realtimeToolSchema{
		...
		realtimeTool("control_shared_app_window", "Operate the currently shared existing macOS app/window through the host Computer Use adapter. Use when the user asks you to click, type, draw, edit, scroll, switch tools, or otherwise manipulate an already shared app such as Pencil, VS Code, Chrome, Notion, or Terminal. By default this queues the app-control work asynchronously and returns a job_id immediately so voice turns do not block; call again with job_id to check status. Structured operations are required: use a state operation first when you need the current window snapshot, then send click/type_text/press_key/scroll/drag operations. A state-only result is not completion; use it to choose the next primitive operation. Do not use this to create a new browser workspace.", objectSchema(nil, map[string]realtimeJSONSchema{
			...
			"processId":        integerSchema("Optional process id from list_shareable_windows.", nil),
			"operations": arraySchema("Explicit primitive app-control operations. Use kind=state to observe the shared window, then use click/type_text/press_key/scroll/drag for direct manipulation instead of sending only a natural-language instruction.", objectSchema([]string{"kind"}, map[string]realtimeJSONSchema{
