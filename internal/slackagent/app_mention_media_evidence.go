...
func renderAppMentionMediaEvidenceSummary(files []SlackThreadFile) string {
	...
	guidance := "Evidence boundary: file metadata is available here; image file_ids can be fetched with slack.fetchImage, and non-image file_ids can be fetched with slack.fetchFile for worker-side readers when relevant. Inspect returned local_path values; Slack file URLs are protected and require the bot token. Do not claim to have watched videos or read binary file contents unless a separate tool result provides that content. For media inventory requests, summarize metadata-backed candidates only when that is useful; do not post a visible reply whose main content is a content-reading blocker."
