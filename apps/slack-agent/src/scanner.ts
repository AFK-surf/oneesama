...
import { buildSlackOAuthAuthorizeUrl, validateSlackAppManifest } from "../../../packages/core/src/slack/slack-app-manifest.js";

type SlackHandlerInput = Record<string, any>;
type SlackChannelLike = Record<string, any>;
type SlackMessageLike = Record<string, any>;
interface SlackApiPageResult<T = unknown> { ok: boolean; items?: T[]; error?: string; method?: string; fixture?: boolean; detail?: unknown; status?: number; source?: string; messages?: T[]; }

export function createSlackScannerHandlers(ctx: any) {
  const { config, expectedSlackManifest, loadConfiguredSlackManifest, slackDomainStore, runner, slackInbound, slackWorkspaceDir, bufferSlackMessageEvent, flushSlackMessageBuffer } = ctx;
