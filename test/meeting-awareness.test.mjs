...
test("meeting awareness prefers fresh caption speaker and merges participant sources", () => {
  ...
  assert.equal("text" in awareness.activeSpeaker, false);
  assert.match(meetingAwarenessContextText(awareness), /当前或最近说话的人：Peng Xiao/);
  assert.doesNotMatch(meetingAwarenessContextText(awareness), /我们继续看参会者信息/);
  assert.doesNotMatch(
    meetingAwarenessContextText(awareness),
    /source=|confidence=|canonical_name=/,
  );
});
...
test("meeting awareness links active speaker aliases to current user", () => {
  ...
  assert.doesNotMatch(
    meetingAwarenessContextText(awareness),
    /canonical_name=|is_current_user=|confidence=/,
  );
});
