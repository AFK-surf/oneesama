...
export async function enableMeetCaptions(
  ...
  const direct: CaptionToggleProbe = await page
    .evaluate(() => {
      ...
      const candidates = nodes
        ...
        .filter(
          (button) =>
            button.visible && /caption|captions|字幕/i.test(`${button.aria} ${button.text}`),
        );
      const alreadyOn = candidates.find((button) =>
        /turn off|hide captions|关闭字幕|停用字幕/i.test(`${button.aria} ${button.text}`),
      );
      const turnOn =
        candidates.find((button) =>
          /turn on|show captions|开启字幕|显示字幕/i.test(`${button.aria} ${button.text}`),
        ) || candidates[0];
      return {
        ok: true,
  ...
  if (direct.ok && direct.alreadyOn) {
    if (!wantsLanguage)
      return { ok: true, method: "direct", alreadyOn: true, candidates: direct.candidates };
    const settings = await enableCaptionsViaSettings(page, captionLanguage, diagnostics, {
  ...
  if (direct.ok && direct.candidateIndex >= 0) {
    const clicked = await page
      .evaluate((index) => {
        const node = Array.from(document.querySelectorAll<HTMLElement>("button, [role=button]"))[
          index
        ];
        if (!node) return false;
        node.click();
        return true;
      }, direct.candidateIndex)
      .catch(() => false);
    if (clicked) {
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    try {
      ...
      if (captionLanguage) {
        ...
        diagnostics?.record(
          "caption_settings_inventory_before_language",
          await inspectCaptionSettingsPanel(page).catch((error) => ({
        ...
        diagnostics?.record(
          "caption_settings_inventory_after_language",
          await inspectCaptionSettingsPanel(page).catch((error) => ({
            error: String(error?.message || error),
          })),
        );
        ...
        diagnostics?.record("caption_settings_live_radio_selected", {
          attempt: attempt + 1,
          selected: await liveCaptionsRadioSelected(page),
        });
      }

      if (enableLiveCaptions) {
        await page
          .locator("text=/Live captions|实时字幕/i")
          .first()
          .click({ timeout: 2500 })
          .catch(() => {});
      }
      await page
        .locator('[aria-label="Close" i], [aria-label="Close dialog" i], [aria-label="关闭"]')
        .first()
        .click({ timeout: 1500 })
        .catch(async () => {
      ...
      diagnostics?.record("caption_settings_attempt_failed", {
        attempt: attempt + 1,
...
async function openLegacyCaptionSettings(page: import("playwright").Page): Promise<"legacy"> {
  await page
    .locator('button[aria-label="More options"], button[aria-label="更多选项"]')
    .first()
    .click({ timeout: 3500 });
  await page
...
async function clickCaptionLanguageCombobox(page: import("playwright").Page): Promise<void> {
  ...
  const candidates = [
    ...
    page
      .locator(
        '[role="combobox"][aria-label*="Language of the meeting" i], [role="combobox"][aria-label*="Meeting language" i], [role="combobox"][aria-label*="会议语言" i], [role="combobox"][aria-label*="會議語言" i]',
      )
      .first(),
  ];
  let lastError: unknown = null;
...
async function clickCaptionLanguageOption(
  page: import("playwright").Page,
  captionLanguage: string,
): Promise<void> {
  ...
  return page
    .evaluate((patternSource: string) => {
      ...
      const translationPattern =
        /your preferred language|translated captions|translate captions|首选语言|偏好語言|翻译字幕|翻譯字幕/i;
      ...
      const textOf = (node: HTMLElement) =>
        (node.innerText || node.textContent || "").replace(/\s+/g, " ").trim();
      // Find currently-expanded listbox that is anchored to the meeting language combobox.
      const expandedCombo = Array.from(
        document.querySelectorAll<HTMLElement>('[role="combobox"][aria-expanded="true"]'),
      )
        .filter(visible)
        .find(
          (combo) =>
            meetingPattern.test(`${textOf(combo)} ${combo.getAttribute("aria-label") || ""}`) &&
      ...
      const candidateOptions = Array.from(
        (scopeRoot || document).querySelectorAll<HTMLElement>('[role="option"]'),
      )
        .filter(visible)
...
async function selectLiveCaptionsMode(page: import("playwright").Page): Promise<void> {
    ...
    .evaluate(() => {
      ...
      for (const label of labels) {
        ...
        for (let depth = 0; parent && depth < 4; depth += 1, parent = parent.parentElement) {
          const radios = parent.querySelectorAll<HTMLElement>(
  ...
  const directChoices = [
    page
      .locator("label")
      .filter({ hasText: /^Live captions$/i })
      .first(),
    page
      .locator('[role="radio"]')
      .filter({ hasText: /^Live captions$/i })
  ...
  const captionType = page
    .locator(
      '[role="combobox"][aria-label*="Caption type" i], [role="combobox"][aria-label*="字幕类型" i], [role="combobox"][aria-label*="字幕類型" i]',
    )
    .first();
  try {
    await clickLocatorRobust(captionType, { timeout: 800 });
    await clickLocatorRobust(
      page
        .locator('[role="option"]')
        .filter({ hasText: /Live captions|实时字幕|即時字幕/i })
        .first(),
      { timeout: 1000 },
    );
  } catch {
      ...
      for (const label of labels) {
        ...
        for (const candidate of candidates) {
          const checked =
            ...
            (candidate as HTMLInputElement).checked?.toString();
  ...
  return page
    .evaluate(() => {
      ...
      const labels = Array.from(
        document.querySelectorAll<HTMLElement>("label, [role='radio']"),
      ).filter((node) => targetPattern.test((node.innerText || node.textContent || "").trim()));
      for (const label of labels) {
        const forId = label.getAttribute("for");
        const candidates: HTMLElement[] = [];
    ...
    return {
      ...
      labels: Array.from(document.querySelectorAll<HTMLElement>("div, span, label, button"))
        .filter(
          (node) =>
            visible(node) &&
            interesting.test(`${textOf(node)} ${node.getAttribute("aria-label") || ""}`),
        )
        .filter(
          (node) =>
            !Array.from(node.children).some((child) =>
              interesting.test(textOf(child as HTMLElement)),
            ),
        )
        .slice(0, 80)
        .map(compact),
      comboboxes: Array.from(document.querySelectorAll<HTMLElement>('[role="combobox"]'))
        .filter(visible)
        .map(compact),
      toggles: Array.from(
        document.querySelectorAll<HTMLElement>(
          '[role="switch"], [role="checkbox"], button[aria-pressed], input[type="checkbox"]',
        ),
      )
        .filter(visible)
        .map(compact),
...
async function clickMeetingLanguageComboboxByLabel(
  page: import("playwright").Page,
): Promise<boolean> {
  return page
    .evaluate(() => {
      ...
      const translationPattern =
        /translated captions|translate captions|translation|your preferred language|caption type|翻译字幕|翻譯字幕|首选语言|偏好語言/i;
      const textOf = (node: HTMLElement) =>
        (node.innerText || node.textContent || "").replace(/\s+/g, " ").trim();
      const visible = (node: HTMLElement) => {
        const rect = node.getBoundingClientRect();
      ...
      const labels = Array.from(document.querySelectorAll<HTMLElement>("div, span, label"))
        .filter(
          (node) =>
            visible(node) &&
            labelPattern.test(textOf(node)) &&
            !translationPattern.test(textOf(node)),
        )
        .filter(
          (node) =>
            !Array.from(node.children).some((child) =>
              labelPattern.test(textOf(child as HTMLElement)),
            ),
        )
      ...
      for (const label of labels) {
        ...
        const combos = Array.from(document.querySelectorAll<HTMLElement>('[role="combobox"]'))
          .filter((combo) => visible(combo))
          .filter((combo) =>
            Boolean(label.compareDocumentPosition(combo) & Node.DOCUMENT_POSITION_FOLLOWING),
          )
          .map((combo) => {
            const rect = combo.getBoundingClientRect();
            ...
            return {
              ...
              text,
              score:
                Math.abs(rect.top - labelRect.top) * 10 + Math.max(0, rect.left - labelRect.left),
            };
          })
...
async function disableTranslatedCaptions(page: import("playwright").Page): Promise<void> {
  await page
    .evaluate(() => {
      const translationPattern =
        /translated captions|translate captions|translation|翻译字幕|翻譯字幕/i;
      const textOf = (node: HTMLElement) =>
        (node.innerText || node.textContent || "").replace(/\s+/g, " ").trim();
      const visible = (node: HTMLElement) => {
        const rect = node.getBoundingClientRect();
    ...
    .evaluate(
      ({ containers, speakerBadge }) => {
        ...
        function captionUiText(text: string): boolean {
          ...
          if (
            /^(English|Chinese, Mandarin \(Simplified\)|Chinese \(Simplified\)|Chinese \(Traditional\)|Japanese|Korean|French|German|Spanish|Portuguese|Italian|Dutch|Russian)$/i.test(
              normalized,
            )
          )
            return true;
          if (
            /^(language|english|closed_caption|live captions|format_size|font size|circle|font color|settings|open caption settings|groups)$/i.test(
              normalized,
            )
          )
            return true;
          if (
            /^(gemini|take notes with gemini|pen_spark|adaptive_audio_mic|domain_disabled)$/i.test(
              normalized,
            )
          )
            return true;
          if (
            /^(press down arrow|external participants joined|your audio is merged with nearby devices)/i.test(
              normalized,
            )
          )
            return true;
          if (
            /^(meeting tools|more options|leave call|turn on microphone|turn off microphone|turn on camera|turn off camera)$/i.test(
              normalized,
            )
          )
            return true;
        ...
        function chooseFallbackSpeaker(lines: string[]): string {
          ...
          return (
            Array.from(counts.entries()).toSorted(
        ...
        function lineCaptionCandidate(line: string, speaker: string): boolean {
          ...
          if (
            !normalized ||
            normalized === speaker ||
            captionUiText(normalized) ||
            aggregateLabel(normalized)
          )
            return false;
        ...
        function shouldForwardToLocalDialog(event: CaptionEvent): boolean {
          ...
          if (!["caption", "captions", "meet-caption", "google-meet-caption"].includes(provider))
            return false;
          ...
          if (
            speaker &&
            botName &&
            (speaker === botName || speaker.includes(botName) || botName.includes(speaker))
          ) {
            return false;
          }
          return true;
          ...
          for (const container of captionContainers) {
            ...
            for (const child of children) {
              ...
              const candidates = Array.from(child.querySelectorAll<HTMLElement>("div, span"))
                ...
                .filter(
                  (el) =>
                    !el.matches(speakerBadge) &&
                    !el.closest(speakerBadge) &&
                    !el.querySelector(speakerBadge),
                )
                .filter(
                  (el) =>
              ...
              if (state.captions.length > 100)
                state.captions.splice(0, state.captions.length - 100);
      ...
      { containers: CAPTION_CONTAINERS, speakerBadge: SPEAKER_BADGE },
    )
    .catch((error) => ({ ok: false, error: String(error?.message || error) }));

  async function browserState(): Promise<CaptionCaptureState | { ok: false; error: string }> {
    return await page
      .evaluate(() => window.MAB_CAPTION_CAPTURE || null)
      .catch((error) => ({
        ok: false,
        error: String(error?.message || error),
      }));
