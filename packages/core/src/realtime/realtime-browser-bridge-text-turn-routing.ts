...
function preferredToolChoiceForTextTurn(text: string) {
  ...
  if (config.directTextTurnToolRouting) {
    ...
    if (appControlIntent && hasConfiguredRealtimeTool("control_shared_app_window")) {
      return { type: "function", name: "control_shared_app_window" };
    }
...
function queueDirectTextTurnTool(name: string, text: string) {
  const callId = `manual_text_turn_${randomEventId()}`;
  recordTimeline("realtime_text_turn_direct_tool_route", {
    name,
    callId,
  });
  Promise.resolve()
    ...
    .then((result) => {
      recordTimeline("realtime_text_turn_direct_tool_done", {
        name,
        callId,
    ...
    .catch((error) => {
      recordTimeline("realtime_text_turn_direct_tool_error", {
        name,
        callId,
...
function forcedToolInstructionsForTextTurn(
  ...
  if (name === "delegate_to_worker") {
    return [
      ...
      "Put the user's lookup request in the task argument and preserve repo, issue, URL, and keyword wording.",
      "Do not say progress text or promise to search before the tool call.",
      `Latest user request: ${userText}`,
    ].join("\n");
  }
  if (name === "stop_video_stage") {
    return [
      ...
      "Do not call share_existing_app_window, list_shareable_windows, kwwk_computer_use, or control_shared_app_window for a stop-share request.",
      `Latest user request: ${userText}`,
    ].join("\n");
  }
  if (name === "kwwk_computer_use") {
  ...
  if (name === "control_shared_app_window") {
    return [
      "For this turn, call control_shared_app_window now before any assistant text.",
      "Put the user's requested app/window action in the instruction argument as natural language.",
      "Do not include low-level operations or coordinate primitives.",
      "Do not use browser/demo workspace tools for an already shared app/window.",
      `Latest user request: ${userText}`,
