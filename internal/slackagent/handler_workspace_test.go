...
func TestHandleWorkspaceBootstrapCreatesTemplates(t *testing.T) {
	...
	if !strings.Contains(response.Body.String(), `"template_count":45`) || !strings.Contains(response.Body.String(), `app_mention_media_request_keywords.en.tmpl`) || !strings.Contains(response.Body.String(), `backfill_agent_read_prompt.en.tmpl`) || !strings.Contains(response.Body.String(), `related_memory_evidence.zh.tmpl`) || !strings.Contains(response.Body.String(), `triage_timeout_summary.zh.tmpl`) || !strings.Contains(response.Body.String(), `delayed_no_reply_deferred_keywords.en.tmpl`) {
