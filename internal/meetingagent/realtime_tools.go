...
func defaultRealtimeToolDefinitions() []realtimeToolSchema {
	return []realtimeToolSchema{
		...
		realtimeTool("control_shared_app_window", "Operate the currently shared existing macOS app/window through the host Computer Use executor. Use when the user asks you to click, type, draw, edit, scroll, switch tools, or otherwise manipulate an already shared app such as Pencil, VS Code, Chrome, Notion, or Terminal. Pass the user's goal as instruction; the host executor owns the observe -> plan -> act -> verify loop, including unfamiliar apps. By default this queues the app-control work asynchronously and returns a job_id immediately so voice turns do not block; call again with job_id to check status. Do not invent click/drag primitives in the foreground Realtime turn and do not use this to create a new browser workspace.", objectSchema(nil, map[string]realtimeJSONSchema{
