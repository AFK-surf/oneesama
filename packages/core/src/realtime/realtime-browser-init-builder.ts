...
function buildMeetSurfaceRealtimePlaceholder(config: Record<string, unknown>) {
  ...
  return [
    ...
    "    const surfaceAudioInput = { enabled: true, streams: 0, chunks: 0, samples: 0, droppedChunks: 0, droppedSamples: 0, inFlight: 0, lastChunkAt: '', errors: [] };",
    ...
    "      return audioContext;",
    "    };",
    "    const forwardSamples = (samples, label) => {",
    ...
    "        return;",
    "      }",
    "      const payload = { sessionId: String(config.sessionId || ''), source: 'host_meet_audio_pcm', label, sampleRate: audioContext ? audioContext.sampleRate : 48000, channels: 1, samples: Array.from(samples, (sample) => Number(sample || 0)) };",
