...
| Gate 3: Slack Assistant / mention behavior | Assistant status lifecycle (`Thinking...`, progress, clear), suggested prompts, suppressed immediate worker ack, rich thread context. | R13 assistant status client/lifecycle, prompt setting, worker callbacks; R14 rich context and hidden Go-only commands; R31 removes provider/worker words from user-visible prompts. | Assistant status tests, app mention work-routing tests, no immediate provider-specific text, terminal result posts once. |
| Gate 4: Slack user command surface | Text commands for join/status/stop/help; slash menu was product-removed. Natural-language mentions are the work surface. | R12/R14/R31 command surface matches the agreed public help; join/stop call meeting-agent; slash command omitted from manifest; background work routing is internal. | Golden help text; app mention/DM command tests; manifest has no `slash_commands` and no `commands` scope. |
...
7. Live spot checks after reviewer approval, not before:
   ...
   - app mention unknown text creates a worker job without an immediate noisy
     provider-specific progress post.
